...
class my_LogisticRegression:
    ...
    def fit(self, X, y, theta,weights):
...
def do_expr(X_all, y_all, name, report_dict):
    ...
    for iMC in range(NMC):
        ...
        NClustersInTest = int(X_test.shape[0])//10
        
        clf_KMeans_train = KMeans(n_clusters=NClustersInTrain).fit(X)
        clf_KMeans_test = KMeans(n_clusters=NClustersInTest).fit(X_test)
    ...
    np.save('Accuracy_convshift_' + name + '_GB_train', acc_GB_train)
    np.save('Accuracy_convshift_' + name + '_GB_test', acc_GB_test)
    np.save('Accuracy_convshift_' + name + '_LR_train', acc_LR_train)
    np.save('Accuracy_convshift_' + name + '_LR_test', acc_LR_test)   
    np.save('Accuracy_convshift_' + name + '_ourGB_train', acc_OurGB_train)
    np.save('Accuracy_convshift_' + name + '_ourGB_test', acc_OurGB_test)
    ...
    plt.legend(loc=4)
    plt.savefig('Accuracy_convshift_' + name + '.png')
